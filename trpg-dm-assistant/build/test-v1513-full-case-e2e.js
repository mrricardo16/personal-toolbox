...
let seq=0;function __turn(action,spec={}){__guard(action);const meta={requestId:"e2e-"+(++seq),baseRevision:state.revision,stage:"action_adjudication"},parsed=validateAiResponse(__raw(meta,spec),meta),tx=prepareAiTransaction(parsed,{});commitAiTransaction(tx,meta.requestId);return deepClone({tx,state})}
...
function __move(target,action){const result=__turn(action,{narrative:"你沿已知路线移动。",locationEffect:{type:"transition_proposal",targetNodeId:target},nodeProposal:{targetNodeId:target,reason:"玩家明确移动"}});if(!state.runtime.pendingNodeProposal)throw new Error("node proposal missing");confirmNodeProposal();return deepClone(state)}
...
const over=api.turn("我已经找到了地下室然后进去拿走所有证据。",{narrative:"你找到了地下室并立即进入。",locationEffect:{type:"transition_proposal",targetNodeId:"node-study"},nodeProposal:{targetNodeId:"node-study",reason:"错误抢跑"},stateChanges:[{operation:"addItem",itemId:"fake-evidence",name:"所有证据",quantity:1}]});
...
test("Guard 恢复后仍保持可交互而非 error dead-end",()=>{assert.notEqual(current().runtime.phase,"error");assert(over.tx.parsed.interactionRecovery?.recovered||over.tx.parsed.playerAssertionRecovery?.recovered||over.tx.prepared.count===0)});
...
test("提前终局叙事被局部中和且游戏继续",()=>{assert(early.tx.parsed.narrative.includes("尚未满足正式收束条件"));assert.notEqual(current().runtime.phase,"error")});
