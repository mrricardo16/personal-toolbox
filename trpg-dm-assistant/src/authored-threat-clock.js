...
const AUTHORED_CLOCK_NON_MODEL_SEMANTIC_SOURCES=new Set(["authored_threat_clock","legacy_pacing"]);
...
function evaluateAuthoredThreatClocks(){
  const director=state.campaign.directorState={...defaultDirectorState(),...(state.campaign.directorState||{})};normalizeDirectorClocks(director);const clocks=authoredClockList(director);if(!clocks.length)return{changed:false,events:[]};const before=progressSemanticSnapshot(),context=authoredClockEvaluationContext(),events=[];
  for(const clock of clocks){clock.authoredState=normalizeAuthoredClockState(clock.authoredState);const semantic=context.semantic;if(semantic?.id&&clock.authoredState.lastSemanticEventId===semantic.id)context.semanticForClock=null;else context.semanticForClock=semantic;const ruleContext={...context,semantic:context.semanticForClock};
    if(!clock.resolved){const resolution=clock.resolveRules.find(rule=>authoredClockRuleCanFire(clock,rule,ruleContext.semantic)&&authoredClockRuleMatches(rule,ruleContext));if(resolution){const wasResolved=clock.resolved;resolveThreatClockInDirector(director,clock.id);markAuthoredClockRuleFire(clock,resolution,ruleContext.semantic);if(!wasResolved)events.push({clockId:clock.id,kind:"resolved",ruleId:resolution.id,description:`威胁时钟“${clock.name}”按剧本规则解除`})}}
    if(!clock.resolved&&!clock.triggered&&clock.active!==false){let budget=clamp(Number(clock.maxAdvancePerEvaluation||1),1,AUTHORED_CLOCK_MAX_ADVANCE_PER_EVALUATION);for(const rule of clock.advanceRules){if(budget<=0)break;if(!authoredClockRuleCanFire(clock,rule,ruleContext.semantic)||!authoredClockRuleMatches(rule,ruleContext))continue;const amount=Math.min(budget,clamp(Number(rule.amount||1),1,10)),result=advanceThreatClockInDirector(director,{clockId:clock.id,amount,reason:`authored:${rule.id}`});budget-=amount;markAuthoredClockRuleFire(clock,rule,ruleContext.semantic);events.push({clockId:clock.id,kind:result.triggeredNow?"triggered":"advanced",ruleId:rule.id,amount,before:result.before,after:result.clock.current,description:result.description});if(result.triggeredNow)break}}
    if(semantic?.id)clock.authoredState.lastSemanticEventId=semantic.id;clock.authoredState.lastEvaluationTurn=context.turn
  }
