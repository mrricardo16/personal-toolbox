...
- v1.5.3 修复线索来源与本轮检定误绑定；内置隐藏线索使用显式获取路线，同时继续允许没有线索、状态或进度的自然叙事回合。
...
- v1.5.1 稳定调查主循环：允许无收益行动，加入 NPC 长期连续性和上下文优先级治理，减少长团中的强行推进与遗忘。

版本记录：

- v1.5.3：修复线索来源误绑定，显式化内置线索路线，保持无收益行动与严格线索保护并存。
