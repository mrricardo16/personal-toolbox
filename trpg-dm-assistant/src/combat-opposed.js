...
function combatStart({opponents=[]}={}){
  ...
  if(healthStabilizationCanAdvanceDying(state.character))state.campaign.combat.playerDyingObservedRound=1;bumpRevision();addLog("combat",`Combat Mode 开始：${order.map(id=>combatParticipant(id)?.label||id).join(" → ")}`,{});renderAll();return combatSnapshot()
}
