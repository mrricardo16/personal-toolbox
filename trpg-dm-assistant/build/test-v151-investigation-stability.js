...
test("版本不低于 v1.5.1",()=>{const match=library.match(/const APP_VERSION = "(\d+)\.(\d+)\.(\d+)";/);assert.ok(match);const [,major,minor,patch]=match.map(Number);assert.ok(major>1||(major===1&&(minor>5||(minor===5&&patch>=1))))});
...
const impactEnd=scenario.includes("const MAX_WORLD_CONTINUITY_EVENTS=12;")?"const MAX_WORLD_CONTINUITY_EVENTS=12;":"function pacingDirective";const impactCode=extract("function classifyTurnImpact(parsed)",impactEnd);vm.runInNewContext(impactCode+"\n;globalThis.classifyTurnImpact=classifyTurnImpact;",sandbox);const classify=sandbox.classifyTurnImpact;
...
test("5 轮无进展让世界行动但不送答案",()=>{const pacing=extract("function pacingDirective()","function buildContextSnapshot");assert.ok(pacing.includes("不得为了推进而强送关键线索")||pacing.includes("不得为了推进而强送关键线索或正确答案"))});
...
test("Lore 优先于最近消息分配预算",()=>{const lore=contextFn.indexOf("loreAllowance"),recent=Math.max(contextFn.indexOf("selectRecentMessagesForContext"),contextFn.indexOf("while(recent.length"));assert.ok(lore>=0&&recent>lore)});
