...
function normalizeFailureForwardCost(rawCost){
  ...
  const tensionRaw=failureForwardCostScalar(cost.tension,"tension",6),hp=failureForwardCostScalar(cost.hp,"hp",100)??0,san=failureForwardCostScalar(cost.san,"san",100)??0,progress=failureForwardCostScalar(cost.progress,"progress",100)??0,resources=failureForwardCostResources(cost.resources);
...
function failureForwardCostAppliedBundle(authored,record){
  const tension=authored.tension>0?(record?.rank==="fumble"?Math.max(2,authored.tension):authored.tension):0,currentHp=Math.max(0,Number(state.character?.hp||0)),currentSan=Math.max(0,Number(state.character?.san||0)),currentProgress=Math.max(0,Number(state.campaign?.progress||0));
  const resources={};for(const [id,amount] of Object.entries(authored.resources||{})){const current=Math.max(0,Number(state.resources?.[id]||0));resources[id]=Math.min(amount,current)}
  return{tension:Math.min(tension,Math.max(0,6-Number(state.campaign?.tension||0))),hp:Math.min(authored.hp,Math.max(0,currentHp-1)),san:Math.min(authored.san,currentSan),progress:Math.min(authored.progress,currentProgress),resources}
}
function failureForwardCostMerge(target,source){target.tension=Math.min(6,target.tension+source.tension);target.hp+=source.hp;target.san+=source.san;target.progress=Math.min(100,target.progress+source.progress);for(const [id,amount] of Object.entries(source.resources||{}))target.resources[id]=(target.resources[id]||0)+amount;return target}
function failureForwardCostPlan(parsed,record){
  const routes=failureForwardCostRouteEntries(parsed,record),totals={tension:0,hp:0,san:0,progress:0,resources:{}},details=[];for(const entry of routes){const applied=failureForwardCostAppliedBundle(entry.cost,record);failureForwardCostMerge(totals,applied);details.push({...deepClone(entry),applied})}
  const effects=[];if(totals.hp>0)effects.push({channel:"state",effect:{operation:"adjustHp",amount:-totals.hp,reason:"失败前进的作者成本"}});if(totals.san>0)effects.push({channel:"state",effect:{operation:"adjustSan",amount:-totals.san,reason:"失败前进的作者成本"}});for(const [resource,amount] of Object.entries(totals.resources))if(amount>0)effects.push({channel:"state",effect:{operation:"adjustResource",resource,amount:-amount,reason:"失败前进的作者成本"}});if(totals.tension>0)effects.push({channel:"campaign",effect:{operation:"adjustTension",amount:totals.tension,reason:"失败前进的作者成本"}});if(totals.progress>0)effects.push({channel:"campaign",effect:{operation:"adjustProgress",amount:-totals.progress,reason:"失败前进的作者成本"}});
  return{version:FAILURE_FORWARD_COST_ENGINE_VERSION,authority:FAILURE_FORWARD_COST_AUTHORITY,recordId:record?.id||null,outcomeContractId:record?.outcomeContract?.contractId||null,immutable:true,policy:"authored_cost_browser_applied",routes:details,totals,effects,createdAt:nowIso()}
}
